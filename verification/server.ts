...
export async function startServer(
  ...
  const server = createServer(async (request, response) => {
    ...
    if (pathname === "/atlas") {
      response.writeHead(308, { Location: "/atlas/" });
      response.end();
      return;
    }
    if (!pathname.startsWith("/atlas/")) {
      respond(response, 404, "Expected /atlas/ subpath");
      return;
    }
    const filename = resolve(
      directory,
      pathname.slice("/atlas/".length) || "index.html",
    );
    if (!filename.startsWith(`${directory}${sep}`)) {
      respond(response, 403, "Path outside dist");
      return;
    }
    try {
      const info = await stat(filename);
      if (!info.isFile()) {
        respond(response, 404, `Not a file: ${pathname}`);
        return;
      }
      ...
      if (
        ...
        if (!bytes) {
          bytes = gzipSync(await readFile(filename));
          compressed.set(filename, bytes);
        }
...
if (
  ...
  const server = await startServer(
    process.argv[2] ?? "dist",
    Number(process.argv[3] ?? 4178),
    process.argv[4],
  );
  console.log(`Static atlas ready: ${server.origin}/atlas/`);

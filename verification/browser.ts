...
export async function instrument(
  ...
  page.on("console", (message) => {
    if (message.type() === "error")
      log.errors.push(`console: ${message.text()}`);
  });
  ...
  context.on("response", (response) => {
    if (response.status() >= 400)
      log.errors.push(`HTTP ${response.status()} ${response.url()}`);
  });
    ...
    new PerformanceObserver((entries) => {
      for (const entry of entries.getEntries()) {
        if (entry.name === "first-contentful-paint")
          target.__atlasMetrics.fcp = entry.startTime;
      }
  ...
  // Observe every real request through Playwright + CDP without pausing it.
  // Paused requests race navigation cancellation (Invalid InterceptionId), while
  // route() disables cache. Unexpected origins are still hard failures in the report.
  ...
  cdp.on("Network.loadingFinished", (event: { encodedDataLength: number }) => {
    log.bytes += event.encodedDataLength;
  });
  return { cdp, log };
}
  ...
  try {
    await page.waitForFunction(
      () => {
        ...
        return (
          atlas &&
          typeof atlas.select === "function" &&
          typeof atlas.reset === "function" &&
          atlas.map &&
          typeof atlas.map.queryRenderedFeatures === "function" &&
          atlas.map.loaded()
        );
      },
      undefined,
    ...
    throw new Error(
      "Application contract unavailable after 15000ms: expected window.__atlas.map (loaded MapLibre Map), select(id), and reset()",
      { cause: error },
    );
  }
    ...
    () =>
      new Promise<number>((accept, reject) => {
        ...
        const timeout = window.setTimeout(() => {
          map.off("idle", done);
          reject(new Error("Map did not emit idle within 15000ms"));
        }, 15000);
        function done() {
          window.clearTimeout(timeout);
          map.off("idle", done);
          accept(performance.now() - start);
        }
        map.on("idle", done);
        map.triggerRepaint();
      }),
  ...
  try {
    ...
    const complete = new Promise<void>((accept, reject) => {
      const timeout = setTimeout(
        () =>
          reject(
            new Error("CDP Tracing.tracingComplete timed out after 15000ms"),
          ),
        15000,
      );
...
export function frameDurations(events: TraceEvent[]): number[] {
  ...
  if (times.length < 30)
    throw new Error(
      `CDP trace contains only ${times.length} DrawFrame timestamps on its busiest compositor thread; at least 30 required`,
    );
  return times
    .slice(1)
    .map((timestamp, index) => (timestamp - times[index]) / 1000);
}

export function percentile(values: number[], quantile: number): number {
  if (values.length === 0 || values.some((value) => !Number.isFinite(value)))
    throw new Error("Missing or non-finite measurement samples");

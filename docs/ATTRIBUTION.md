...
Fraunces (Undercase Type, Phaedra Charles and Flavia Zimbardi) and Inter (Rasmus Andersson) are self-hosted WOFF2, SIL Open Font Licence 1.1. Full notices are retained in `public/fonts/OFL-fraunces.txt` and `OFL-inter.txt`. Noto Sans map glyphs retain `public/glyphs/OFL.txt`. MapLibre GL JS and PMTiles are BSD-3-Clause; React is MIT. Runtime notices are shipped in the public directory.
